...
class TournamentDbTest extends Specification {
  ...
  "TournamentDb" should {
    "get a tournament by id" in new WithApplication {
      ...
      Await.result(tournamentDb.insertTournament(tournamentToGet), DEFAULT_DURATION)
    ...
    "return a list of all tournaments" in new WithApplication {
      val tournamentDb = initTournamentDb
      val tournament = Tournament(Some(1),"Clubkampioenschap",LocalDate.of(2015,5,12),0,hasMultipleSeries = false,showClub = false)
      Await.result(tournamentDb.insertTournament(tournament), DEFAULT_DURATION)
      val tournamentList = Await.result(tournamentDb.getAllTournaments, Duration(3000, "millis"))
      tournamentList must beEqualTo(List(tournament))
    }

    "insert a tournament" in new WithApplication{
      val tournamentDb = initTournamentDb

      val rowId = Await.result(tournamentDb.insertTournament(Tournament(None, "Klaaskampioenschap",LocalDate.of(2015,11,23), 0, hasMultipleSeries = false, showClub = false)), Duration(3000, "millis")).map(_.tournamentId).get
      val insertedTournament = Await.result(tournamentDb.getTournament(rowId.get), Duration(3000, "millis"))
      insertedTournament must beSome( Tournament(Some(1), "Klaaskampioenschap",LocalDate.of(2015,11,23), 0, hasMultipleSeries = false, showClub = false))
    }

    "update a tournament" in new WithApplication {
      val tournamentDb = initTournamentDb
      Await.result( tournamentDb.insertTournament(Tournament(None, "Clubkampioenschap", LocalDate.of(2015,4,15), 1, hasMultipleSeries=true, showClub = true)) , DEFAULT_DURATION)
